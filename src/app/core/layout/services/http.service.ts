...
export class HttpService {
  constructor(
    ...
    @Inject('API_URI') private apiUrl: any
  ) {}
  ...
  getBlob(
    ...
    return this.http
      .get(this.getUrl(path, applyBaseApiUrl), {
  ...
  put<T>(
    ...
    return this.http
      .put<T>(this.getUrl(path, applyBaseApiUrl), body)
      .pipe(catchError(this.formatErrors));
  }
  ...
  postResponsText(
    ...
    return this.http.post(this.getUrl(path, applyBaseApiUrl), body, {
      responseType: 'text',
    });
  }
  ...
  delete<T>(path: string, applyBaseApiUrl: boolean = true): Observable<T> {
    return this.http
      .delete<T>(this.getUrl(path, applyBaseApiUrl))
      .pipe(catchError(this.formatErrors));
  }
  ...
  getUrl = (path: string, applyBaseApiUrl: boolean): string => {
    return applyBaseApiUrl ? `${this.apiUrl}${path}` : `${path}`;
  };
}
